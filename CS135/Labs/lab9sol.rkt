...
;(display (compose test study))
;(display (composite test study))
...
(define (removes set)
  (if (equal? set '())
  	...
  	(if (member (car set) (cdr set))
      	(removes (cdr set))
      	(cons (car set) (removes (cdr set))))))
                  	 
                  	 
(define (unique relation)
  (if (equal? relation '())
  	'()
  	(append (count (car relation) '()) (unique (cdr relation)))))

(define (count relation set)
  (if (equal? relation '())
  	'()
  	(if (member (car relation) set)
      	'()
      	(cons (car relation) (count (cdr relation) (cons (car relation) set))))))
