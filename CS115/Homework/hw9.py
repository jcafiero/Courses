...
def letterScoreLoop(letter, scorelist):
    ...
    for x in range(len(scorelist)):
        if letter == scorelist[x][0]:
            return scorelist[x][1]
...
def wordScoreLoop(S, scorelist):
    ...
    for x in range(len(S)):
        if S == '':
            return 0
        score += letterScore(S[x], scorelist)
    return score
...
def wordsWithScoreLoop(dct, scores):
    ...
    for x in range(len(dct)):
         newdct[x] = [dct[x], wordScore(dct[x], scores)]
    return newdct

print wordsWithScoreLoop(['a'], scrabbleScores)