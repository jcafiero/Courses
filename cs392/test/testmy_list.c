...
void test(void * symbols, int size, int kind, void (* printer)(struct s_node * arg), void (* debugger)(struct s_node * arg)){
  ...
  *NULL_HOLDER = NULL_NODE;
  ...
  debugger(*list);
  my_str("\n");
  add_node_at(k, list,1);//this is where it seg faults
  debugger(*list);
  my_str("\n");
  ...
  my_int((t1 = count_s_nodes(*list)));//this case returns 10, not 12
  my_str("\n");
  ...
  if(remove_node_at(NULL_LIST, 3) == NULL)//instant seg fault
